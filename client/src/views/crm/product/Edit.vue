<template>
    <page>
        <page-header header="📦 Edit Product"></page-header>
        <page-body class="p-3" v-if="item">
            ...
            <b-col lg="12">
                <div id="listgroup-ex" style="position:relative; overflow-y:auto; height:auto">
                    <b-card bg-variant="white" text-variant="dark" header="Product SKUS" header-bg-variant="light" header-text-variant="info">
                        <b-row>
                            <b-col>
                                <sku-component v-if="item" :product_id="item.id"></sku-component>
                            </b-col>
                        </b-row>
                    </b-card>
                </div>
...
export default {
    ...
    methods: {
        afterGotData(item) {
            ...
            this.list_images = item.image_url
        },
        // checkProduct() {
        //     const API_URL = process.env.VUE_APP_API_URL
        //     let url = buildURL(API_URL, 'crm/product/check-product/'+ this.item.name)
        ...
        beforeSave() {
            ...
            // if (typeof this.item.image_url !== 'string') {
            //     this.item.image_url = JSON.stringify(this.item.image_url)
            // }
            this.$v.item.$touch()
        ...
        afterSave(item) {
            this.$confirm('Tạo thêm sản phẩm mới?', 'Thông báo', {
