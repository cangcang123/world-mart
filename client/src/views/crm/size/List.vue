<template>
<page>
    <page-header>️👘 Sizes</page-header>

    <action-bar @change="onFilterChanged">
        <b-button :to="{name: 'SizeAdd'}" class=" mdi mdi-plus mb-3 float-right" variant="success" size="sm" slot="header-right">Add Size</b-button>
    </action-bar>
    ...
    <b-table class="bg-white shadow-sm" table-class="mb-0"
        ...
        <template v-slot:cell(options)="data">
            ...
            <b-button-group>
                <b-button id="SizeEdit" :to="{name: 'SizeEdit', params: {id: data.item.id}}" size="sm" variant="warning" class="mdi mdi-circle-edit-outline"></b-button>
                <b-tooltip target="SizeEdit" triggers="hover">Edit</b-tooltip>
            </b-button-group>
        </template>
    </b-table>
