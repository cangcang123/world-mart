<template>
<page>
    <page-header>🔖 Promotions</page-header>

    <action-bar @change="onFilterChanged">
        <b-button :to="{name: 'PromotionAdd'}" class=" mdi mdi-plus mb-3 float-right" variant="success" size="sm" slot="header-right">Add Promotion</b-button>
    </action-bar>
    ...
    <b-table class="bg-white shadow-sm" table-class="mb-0"
        ...
        <template v-slot:cell(options)="data">
            ...
            <b-button-group>
                <b-button id="PromotionEdit" :to="{name: 'PromotionEdit', params: {id: data.item.id}}" size="sm" variant="warning" class="mdi mdi-circle-edit-outline"></b-button>
                <b-tooltip target="PromotionEdit" triggers="hover">Edit</b-tooltip>
            </b-button-group>
        </template>
    </b-table>
